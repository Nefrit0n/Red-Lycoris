...
func NewRouter(pool *pgxpool.Pool, rdb *redis.Client, corsOrigins string, opts ...RouterOption) http.Handler {
	...
	r.Group(func(r chi.Router) {
		...
		r.Route("/api/v1/admin", func(r chi.Router) {
			...
			r.Get("/groups", handleListGroupsAdmin(usersRepo))
			r.Post("/groups", handleCreateGroup(usersRepo, auditWriter))
			r.Get("/groups/{id}", handleGetGroup(usersRepo))
			r.Patch("/groups/{id}", handlePatchGroup(usersRepo))
			r.Delete("/groups/{id}", handleDeleteGroup(usersRepo))
			r.Get("/groups/{id}/members", handleListGroupMembers(usersRepo))
			r.Post("/groups/{id}/members", handleAddGroupMembers(usersRepo))
			r.Delete("/groups/{id}/members/{uid}", handleDeleteGroupMember(usersRepo))
			r.Get("/groups/{id}/projects", handleListGroupProjects(usersRepo))
			r.Put("/groups/{id}/projects/{pid}", handlePutGroupProjectAccess(usersRepo, auditWriter))
			r.Delete("/groups/{id}/projects/{pid}", handleDeleteGroupProjectAccess(usersRepo))

			r.Get("/roles", handleListRolesReadOnly(usersRepo))
			r.Get("/roles/{id}", handleGetRoleReadOnly(usersRepo))
			r.Get("/permissions", handleListPermissions(usersRepo))
