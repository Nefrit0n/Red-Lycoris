...
func parseFindingsFilter(w http.ResponseWriter, r *http.Request, rolesRepo *storage.UserProjectRolesRepo) (storage.FindingsFilter, bool, bool) {
	...
	if q.Get("assignee") == "me" {
		uid := user.ID
		filter.AssigneeMeUserID = &uid
	}
	if q.Get("unassigned") == "true" {
		filter.Unassigned = true
	}
	if v := q.Get("assignees"); v != "" {
		for _, s := range strings.Split(v, ",") {
			id, err := uuid.Parse(strings.TrimSpace(s))
			if err != nil {
				respondError(w, r, http.StatusBadRequest, "VALIDATION_ERROR", "invalid assignees value")
				return filter, false, false
			}
			filter.AssigneeUserIDs = append(filter.AssigneeUserIDs, id)
		}
	}
...
func handleListFindings(repo *storage.FindingsRepo, rolesRepo *storage.UserProjectRolesRepo) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if filter.GroupBy != "" {
			...
			case "cve", "component", "rule", "secret", "cwe":
			default:
