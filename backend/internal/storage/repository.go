...
func createFindingWithExecer(ctx context.Context, ex execer, finding *models.Finding) error {
	...
	_, err := ex.ExecContext(
		ctx,
		`INSERT INTO findings (id, tenant_id, scan_result_id, product_id, fingerprint, category, title, description, severity, severity_rank, status, status_rank, duplicate_id, assignee_id, import_job_id, first_seen_at, last_seen_at, repeat_count, sla_due_at, sla_breached, sla_breached_at, sla_profile, sla_source, source_type, source_version, endpoint_method, endpoint_path, evidence, cwe, owasp, raw_data, created_at, updated_at)
