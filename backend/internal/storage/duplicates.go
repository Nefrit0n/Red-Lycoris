...
func GetFindingDuplicateGroup(ctx context.Context, db *sql.DB, findingID uuid.UUID) (*DuplicateGroup, error) {
	...
	rows, err := db.QueryContext(
		ctx,
		`SELECT f.id, f.tenant_id, f.title, f.description, f.fingerprint, f.severity, f.status, f.product_id, p.name, f.assignee_id, f.import_job_id, f.first_seen_at, f.last_seen_at, f.repeat_count, f.duplicate_id, f.created_at, f.updated_at, f.deleted_at
		 FROM findings f
