...
func TestFindingsListDefaultsToCanonical(t *testing.T) {
	...
	mock.ExpectQuery("SELECT f.id.*duplicate_id IS NULL").
		WillReturnRows(sqlmock.NewRows([]string{"id", "title", "severity", "status", "product_id", "name", "assignee_id", "username", "import_job_id", "created_at", "updated_at", "last_seen_at", "repeat_count", "duplicate_id", "scanner"}))
