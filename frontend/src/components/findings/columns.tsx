...
// Column catalog for the findings table.
//
// Each column is a {header, width (Tailwind width class), Cell component}
// triple. The table shell decides width via flex/shrink classes on the row
// container, and the Cell renders the actual value. `getColumnsForKind`
// picks the subset for the active kind — when nothing is scoped, we fall
// back to the "all" preset which emphasises common columns (severity, title,
// enrichment, project, age).
//
// Cells are pure functions on a Finding — no hooks inside so the row layout
// can re-render efficiently during virtual scroll.

export interface FindingColumn {
  id: string;
  header: string;
  // Tailwind width class applied on BOTH header cell and row cell — keeps
  // them perfectly aligned since both use the same flex rules.
  widthClass: string;
  // min/md/lg visibility — matches the responsive breakpoints used in the
  // table shell for hiding narrow columns on small screens.
...
// Title cell — kind icon + truncated title. Always the first content column.
...
// Kind-specific presets. Severity is drawn as a left border on the row
// itself, so it's not part of the column list.
