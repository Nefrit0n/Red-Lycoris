...
export function SourceCardSuccess({ s }: { s: EnrichmentSourceDTO }) {
  return <div className='rounded-[10px] border border-green-500/20 bg-green-500/5 p-[14px]'>
    ...
    <Sparkline points={s.sparkline_7d}/>
    <div className='text-sm text-zinc-400'>{ru.card.lastSuccess(formatRelativeTime(s.last_sync_at))}</div>
  </div>;
}
