...
def create_scan_run(request):
    # CSRF защищает unsafe-методы (POST/PUT/DELETE), а @csrf_exempt отключает это.
    # Для MVP ок, но позже заменим на нормальную auth (token) и ограничения.
    ...
    # считаем sha256 и одновременно пишем во временный файл
    # Django рекомендует читать загруженный файл через chunks(), чтобы не убить память :contentReference[oaicite:9]{index=9}
    h = hashlib.sha256()
    with tempfile.NamedTemporaryFile("wb+", delete=True) as tmp:
        ...
        bucket = (request.POST.get("bucket") or "raw-reports").strip()
        key = f"{project.id}/{tool_name}/{sha256_hex}/{uploaded.name}"

        upload_fileobj_to_s3(tmp, bucket=bucket, key=key, content_type=getattr(uploaded, "content_type", None))
        raw_uri = f"s3://{bucket}/{key}"

    # idempotency_key = sha256 файла
    ...
    try:
        ...
        # уже есть такой же отчёт > возвращаем существующий ScanRun
        scan_run = ScanRun.objects.get(project=project, tool_name=tool_name, idempotency_key=idempotency_key)
        created = False
    ...
    return JsonResponse(
        {
            ...
            "asset": {"type": sr.asset.asset_type, "identifier": sr.asset.identifier} if sr.asset else None,
